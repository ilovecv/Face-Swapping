...
function [flow, labels] = optimalSeamSearch(im_src_wrap, im_dst, mask_dst, mask_src)
    ...
    pixel_num = size(im_dst,1)*size(im_dst,2);
    ...
    im_shift_dst = padarray(im_dst,[0,1],'pre');
    ...
    M_left = sum((im_src_wrap - im_dst).^2, 3) + sum((im_shift_src - im_shift_dst).^2, 3);

    im_shift_src = padarray(im_src_wrap,[0,1],'post');
    im_shift_dst = padarray(im_dst,[0,1],'post');
    ...
    M_right = sum((im_src_wrap - im_dst).^2, 3) + sum((im_shift_src - im_shift_dst).^2, 3);

    im_shift_src = padarray(im_src_wrap,[1,0],'pre');
    im_shift_dst = padarray(im_dst,[1,0],'pre');
    ...
    M_up = sum((im_src_wrap - im_dst).^2, 3) + sum((im_shift_src - im_shift_dst).^2, 3);

    im_shift_src = padarray(im_src_wrap,[1,0],'post');
    im_shift_dst = padarray(im_dst,[1,0],'post');
    ...
    M_down = sum((im_src_wrap - im_dst).^2, 3) + sum((im_shift_src - im_shift_dst).^2, 3);
